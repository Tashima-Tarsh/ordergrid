...
must(files["cloudflare/worker.mjs"].includes("env.BACKEND.getByName"),"cloudflare contract: container routing binding missing");
...
must(files["cloudflare/worker.mjs"].includes("cloudflare-container"),"cloudflare contract: container backend mode missing");
must(files["cloudflare/worker.mjs"].includes("renderDependency:false"),"cloudflare contract: Render dependency must be removed");
must(!files["cloudflare/worker.mjs"].includes("onrender.com"),"cloudflare contract: Render origin must not remain in Worker");
must(files["wrangler.jsonc"].includes('"class_name": "OrderGridBackend"'),"cloudflare contract: backend container binding missing");
must(files["wrangler.jsonc"].includes('"instance_type": "standard-1"'),"cloudflare contract: browser backend must use 4 GiB container");
must(files["wrangler.jsonc"].includes('"regions": ["APAC"]'),"cloudflare contract: APAC container placement missing");
must(files["wrangler.jsonc"].includes('"crons": ["*/5 * * * *"]'),"cloudflare contract: container keepalive cron missing");
must(server.includes('ORDERGRID_CLOUDFLARE_CONTAINER==="true"'),"cloudflare contract: container runtime defaults missing");
must(server.includes("required only when creating the first OrderGrid owner"),"cloudflare contract: existing workspace must not require bootstrap password at startup");
