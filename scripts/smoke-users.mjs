...
must(files["cloudflare/worker.mjs"].includes('url.pathname.startsWith("/api/")'),"cloudflare free contract: API route handling missing");
must(files["wrangler.jsonc"].includes('"run_worker_first": ["/api", "/api/*"]'),"cloudflare free contract: API must run Worker before SPA assets");
must(files["wrangler.jsonc"].includes('"directory": "./public"'),"cloudflare free contract: public asset directory missing");
must(files["cloudflare/worker.mjs"].includes('plan:"workers-free"'),"cloudflare free contract: free-plan status missing");
must(files["cloudflare/worker.mjs"].includes('from "pg"'),"cloudflare free contract: direct Postgres driver missing");
must(files["cloudflare/worker.mjs"].includes('url.pathname==="/api/login"'),"cloudflare free contract: login route missing");
must(files["cloudflare/worker.mjs"].includes('url.pathname==="/api/dashboard"'),"cloudflare free contract: dashboard route missing");
must(!files["cloudflare/worker.mjs"].includes("onrender.com"),"cloudflare free contract: Render dependency must be absent");
must(!files["wrangler.jsonc"].includes('"containers"'),"cloudflare free contract: paid Containers must be absent");
must(!files["wrangler.jsonc"].includes('"durable_objects"'),"cloudflare free contract: paid container Durable Object binding must be absent");
