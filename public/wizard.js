(()=>{
  ...
  function productEntry(){
    ...
    node.innerHTML='<div class="two"><label>Product URL<input name="url" type="url" required placeholder="https://www.amazon.in/dp/..."></label><label>Estimated unit price (₹)<input name="price" type="number" min="1" step="0.01" value="1299" required></label></div><div class="gst-product-grid"><label>Quantity per recipient<input name="quantity" type="number" min="1" max="100" value="1" required></label><label>HSN / SAC<input name="hsnSac" maxlength="16" placeholder="e.g. 8471" required><small>Use the supplier or retailer tax classification. OrderGrid does not infer HSN/SAC from the product URL.</small></label><label>GST rate<select name="gstRate" required><option value="0">0%</option><option value="0.1">0.1%</option><option value="0.25">0.25%</option><option value="1.5">1.5%</option><option value="3">3%</option><option value="5">5%</option><option value="7.5">7.5%</option><option value="12">12%</option><option value="18" selected>18%</option><option value="28">28%</option><option value="40">40%</option></select></label><label>Cess %<input name="cessRate" type="number" min="0" max="100" step="0.01" value="0"></label><label class="gst-inclusive"><input name="priceIncludesGst" type="checkbox" checked> Price includes GST</label><button type="button" class="secondary remove-product" data-remove-product>Remove product</button></div>';
    return node;
  }
