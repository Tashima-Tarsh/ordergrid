...
app.setErrorHandler((error,req,reply)=>{req.log.error(error);if(error instanceof z.ZodError)return reply.code(400).send({error:"invalid_request",issues:error.issues});const status=Number((error as any)?.statusCode||500);if(status===429)return reply.code(429).send({error:"rate_limited",message:"Too many requests. Please retry shortly."});if(status>=400&&status<500)return reply.code(status).send({error:"request_failed",message:error.message});return reply.code(500).send({error:"internal_error",requestId:req.id});});
