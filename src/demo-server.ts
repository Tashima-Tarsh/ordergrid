...
app.get("/api/execution-workers",async()=>({workers:[]}));
app.get("/api/bulk-baskets",async()=>({baskets:[]}));
app.post("/api/bulk-queue/claim",async(_,reply)=>reply.code(409).send({error:"showroom_preview_only",message:"Bulk execution workers are available on the production OrderGrid API."}));
