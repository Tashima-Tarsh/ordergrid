...
app.post("/api/bulk-queue/:id/commercial-check",async(req,reply)=>{
  ...
  const sameBatch=activeBaskets().filter(b=>b.batch_id===basket.batch_id);
  const expectedBatchMinor=sameBatch.reduce((n,b)=>n+b.amount_minor,0);
  const projectedBatchMinor=sameBatch.reduce((n,b)=>n+(b.id===basket.id?body.amountMinor:(b.observed_amount_minor??b.amount_minor)),0);
  ...
  if(orderVariancePercent>policy.max_price_increase_percent)breaches.push("PRICE_VARIANCE");
  if(policy.max_order_value_minor>0&&body.amountMinor>policy.max_order_value_minor)breaches.push("ORDER_VALUE");
  if(batchVariancePercent>policy.max_batch_variance_percent)breaches.push("BATCH_VARIANCE");
  basket.observed_amount_minor=body.amountMinor;
  basket.commercial_variance_percent=orderVariancePercent;
  if(breaches.length){
    if(policy.price_breach_action==="PAUSE_BATCH"){
      for(const b of sameBatch.filter(b=>["READY","CLAIMED","OPENED","REQUIRES_ACTION"].includes(b.status))){
        b.status="REQUIRES_ACTION";b.commercial_status="REVIEW_REQUIRED";b.failure_code="PRICE_POLICY_REVIEW_REQUIRED";b.failure_message="Commercial policy review required: "+breaches.join(", ");b.execution_worker_id=undefined;b.expires_at=undefined;
      }
    }else{
      basket.status="REQUIRES_ACTION";basket.commercial_status="REVIEW_REQUIRED";basket.failure_code="PRICE_POLICY_REVIEW_REQUIRED";basket.failure_message="Commercial policy review required: "+breaches.join(", ");basket.execution_worker_id=undefined;basket.expires_at=undefined;
    }
    return {allowed:false,status:"REVIEW_REQUIRED",expectedAmountMinor:expectedMinor,observedAmountMinor:body.amountMinor,orderVariancePercent,batchVariancePercent,breaches,action:policy.price_breach_action};
  }
  basket.commercial_status="APPROVED";
