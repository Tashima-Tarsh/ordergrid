...
app.post("/api/bulk-queue/:id/open",async(req,reply)=>{const p=req.principal!,id=z.string().uuid().parse((req.params as any).id);const {rows}=await db.query(`
  ...
  where cb.id=$1 and cb.tenant_id=$2 and cb.claimed_by=$3 and cb.status in ('CLAIMED','OPENED','REQUIRES_ACTION') and (cb.expires_at>now() or cb.status='REQUIRES_ACTION')
