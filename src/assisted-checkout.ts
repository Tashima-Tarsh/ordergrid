...
export type RetailerIdentity = { id:string; name:string; host:string; mode:"ASSISTED" };
...
export function retailerForProductUrl(value:string):RetailerIdentity {
  ...
  if(known) return {id:known.id,name:known.name,host,mode:"ASSISTED"};
  if(/^\/products\/[^/]+/.test(url.pathname)) return {id:`store:${host}`,name:host,host,mode:"ASSISTED"};
  throw new Error("Unsupported retailer product URL");
}

export function checkoutHandoffUrl(value:string) {
